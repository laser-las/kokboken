...
const categories = computed(() => {
  const fromData = Array.from(new Set(recipes.value.map((r) => r.category).filter(Boolean)))
  return ['Alla recept', ...fromData]
})
...
<template>
  <div class="page-container">
    ...
    <main>
      ...
      <section v-else class="recipe-grid" aria-label="Recept">
...
.new-recipe { border: 1px solid #c87050; background: #c87050; color: white; border-radius: 999px; padding: .6rem 1.2rem; font: .72rem Arial, sans-serif; cursor: pointer; margin-bottom: 1.5rem; }
